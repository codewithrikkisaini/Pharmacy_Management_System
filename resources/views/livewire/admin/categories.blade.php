<div class="space-y-8">
    ...
    <div class="bg-white rounded-[32px] card-shadow overflow-hidden">
        <div class="p-8 border-b border-slate-50 flex items-center justify-between">
            <div class="relative">
                <input type="text" wire:model.live="search" placeholder="Search categories..." class="bg-slate-50 border-none rounded-xl px-5 py-3 text-sm w-80 focus:ring-2 focus:ring-sky-500 transition-all">
                    ...
                    <tr class="hover:bg-slate-50/30 transition-colors group">
                        <td class="px-8 py-5">
                            <div class="w-14 h-14 rounded-2xl overflow-hidden shadow-sm border border-slate-100 group-hover:scale-105 transition-transform duration-300">
                                <img src="{{ $category->image ? asset('storage/'.$category->image) : 'https://ui-avatars.com/api/?name='.urlencode($category->name).'&color=0ea5e9&background=e0f2fe' }}" class="w-full h-full object-cover">
                            </div>
                        </td>
                        <td class="px-8 py-5">
                            <div class="font-bold text-slate-900">{{ $category->name }}</div>
                            <div class="text-[10px] font-bold text-sky-500 uppercase tracking-widest mt-1">Slug: {{ $category->slug }}</div>
                        </td>
                        <td class="px-8 py-5">
                        ...
                        <td class="px-8 py-5 text-right space-x-3">
                            <button wire:click="edit({{ $category->id }})" class="text-sky-600 hover:text-sky-700 font-bold text-sm bg-sky-50 px-4 py-2 rounded-xl transition-all">Edit</button>
        ...
        <div class="bg-white w-full max-w-xl rounded-[40px] shadow-2xl overflow-hidden transform transition-all">
            ...
            <form wire:submit.prevent="save" class="p-10 space-y-8">
                <div>
                    ...
                    <input type="text" wire:model="name" class="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-sky-500 font-medium" placeholder="e.g. Tablets, Syrups">
                ...
                <div>
                    ...
                    <textarea wire:model="description" class="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-sky-500 font-medium" rows="4" placeholder="Briefly describe this category..."></textarea>
